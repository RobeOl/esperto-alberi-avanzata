...
**2. Lightbox per le foto — ✅ COMPLETATA E TESTATA**
...
- **Pinch-to-zoom funzionante su telefono/tablet**: due dita sulla foto aperta zoomano fino a 4x (calcolo della distanza tra i due punti di tocco su `touchstart`/`touchmove`/`touchend`); con un dito, una volta zoomati, si trascina l'immagine per spostarsi sui dettagli.
- Aggiunto un bottone **"✕"** in alto a destra nell'overlay per chiudere in modo affidabile anche da zoomato.
- Tap "semplice" di chiusura che **ignora i tap accidentali** arrivati subito dopo un pinch/trascinamento (altrimenti si rischiava di chiudere la foto per errore mentre si stava ancora zoomando).
- Zoom e posizione si resettano automaticamente ogni volta che si apre/chiude una foto.
- `touch-action: none` su overlay e immagine per evitare conflitti tra il gesto custom e lo zoom nativo del browser/WebView.
...
- Funzioni chiave da conoscere: `cambiaTab(tabId)`, `filtraSpecie()`, `eseguiConfronto()`, `esportaReport(alberoId)`, `apriLightbox(src)` / `chiudiLightbox()` / `gestisciClickOverlay()`, `lbApplyTransform()` / `lbResetTransform()` (pinch-zoom).

---

## Prossimo passo suggerito

Schermata iniziale e lightbox sono entrambe completate e testate su PC e mobile. Riprendere dal punto **3 (ricerca testuale nella lista specie)**, oppure chiarire prima i punti in sospeso (6 e 7) se l'utente ha già le idee più chiare su tag CSV e risorse esterne da linkare.
